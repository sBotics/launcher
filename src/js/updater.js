...
var currentWidth = 0;
var variantWidth;
var stringsLoading = ["Verificando arquivos", "Verificando arquivos.", "Verificando arquivos..", "Verificando arquivos..."]
var localsystem = {
};



function RecursiveMkdir(pathe){
    console.log(pathe);
    var array = pathe.split(path.sep);
    console.log(array[0]);
    console.log(!fs.existsSync(array[0]));
    if (array[0] == ""){
        array[0] = "/"
    }
    var new_array;
...
async function download (url, pathe, filename) {  
...
async function unzip (pathe, filename) {  
...
var tries = 0;
        ...
        if (!fs.existsSync(file_path)){
            stringsLoading = ["Baixando "+objf.name +"","Baixando "+objf.name +".","Baixando "+objf.name +"..","Baixando "+objf.name +"..." ];
            tries++;
            await download(objf.download,objf.path, objf.name);
            if (objf.format == "zip"){
                await unzip(objf.path, objf.name.slice(0, objf.name.length-4))
            }
            localsystem[file_path] = {downloaded: luxon.DateTime.local().toUTC().toMillis()}
            localsystem["seses"] = luxon.DateTime.local().toUTC().toMillis();
            fs.writeFileSync(config_path,JSON.stringify(localsystem));
        }
        
        else if (localsystem[file_path].downloaded < ParseTime(objf.last_updated_at)){
    ...
    catch (e)
        {
            ...
            while (true){
                try{
                    stringsLoading = ["Tivemos um problema. Baixando "+objf.name +" novamente","Tivemos um problema. Baixando "+objf.name +" novamente.","Tivemos um problema. Baixando "+objf.name +" novamente" +"..","Tivemos um problema. Baixando "+objf.name +" novamente" +"..." ];
                    if (fs.existsSync(file_path)){
...
stringsLoading = ["Finalizando", "Finalizando.", "Finalizando..", "Finalizando..."];
localsystem["seses"] = luxon.DateTime.local().toUTC().toMillis();
fs.writeFileSync(config_path,JSON.stringify(localsystem));
...
switch (FolderPath){
case 'mac':
    string_execute = path.join(homedir, "sBotics", "sBotics.app", "Contents", "MacOS", "sBotics");
...
fs.chmodSync(path.join(homedir, "sBotics", "sBotics.app", "Contents", "MacOS", "sBotics"), 0o777);
}
...
console.log('https://raw.githubusercontent.com/Txiag/sBotics/master/'+FolderPath+'.json')
axios.get('https://raw.githubusercontent.com/Txiag/sBotics/master/'+FolderPath+'.json').then(main); 


let myVar = setInterval(function(){ timer() }, 500);
var currentString = 0;
function timer(){
document.getElementById("alertDown").innerText = stringsLoading[currentString++];
